...
class ProcessGroup
{
    ...
    public function __construct(Sweatshop $sweatshop, $queueClass, $workerClass, $topics, $options = [])
    {
    ...
    public function syncProcesses()
    {
        ...
        $this->getLogger()->debug('Launching Queue with options', ['queue' => $queueClass, 'options' => $queueOptions]);
        foreach ($this->PIDs as $pid) {
            if (0 == $pid) {
                $process = new ProcessWrapper($this->_di['sweatshop'], $queueClass, [$workerClass => $workerOptions], $queueOptions);
